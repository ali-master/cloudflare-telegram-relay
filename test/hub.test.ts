...
import type { Env, NotificationInput } from '../src/types';
import {hubName} from '../src/types';
import type {TenantLimits, TenantRuntime} from '../src/types';
...
async function subscribe(chatId: number, text = '/start', type = 'private') {
  await hub.handleUpdate({ update_id: ++updateId, message: { chat: { id: chatId, type }, from: { first_name: `User ${chatId}`, username: `user${chatId}` }, text } });
}

async function resume() { await hub.updateSettings({ ...await hub.getSettings(), paused: false }); }
async function tick(milliseconds = 1_001) { now += milliseconds; await runDurableObjectAlarm(hub); }
function accepted() { return new Response(JSON.stringify({ ok: true, result: { message_id: 100 } }), { headers: { 'Content-Type': 'application/json' } }); }
...
beforeEach(async () => {
  ...
  if (!(await registry.getRuntime('default'))?.botToken) {
    const validation = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => new Response(JSON.stringify({ok: true, result: {id: 123456, is_bot: true, first_name: 'Default test bot', username: 'default_test_bot'}})));
...
describe('durable membership and notification acceptance', () => {
  it('subscribes private chats once, durably deduplicates updates, and stops or disables membership', async () => {
    ...
    const start = { update_id: 101, message: { chat: { id: 1, type: 'private' }, from: { first_name: 'Ali' }, text: '/start' } };
    ...
    await subscribe(1);
    await hub.handleUpdate({ update_id: 102, my_chat_member: { chat: { id: 1, type: 'private' }, new_chat_member: { status: 'kicked' } } });
    expect((await hub.listSubscribers(1)).items[0].active).toBe(false);
  });
  ...
  it('fans out atomically to the current subscribers and rejects conflicting idempotency keys', async () => {
    ...
    await subscribe(3);
    const repeated = await hub.enqueue({ ...input, timestamp: '2026-09-12T09:30:00.000Z' }, source, 'build-42', 'stable-request');
    ...
    await expect((async () => await hub.enqueue({ ...input, text: 'Different message' }, source, 'build-42', 'different-request'))()).rejects.toThrow('IDEMPOTENCY_CONFLICT');
...
describe('persistent Telegram delivery queue', () => {
  it('pauses and resumes delivery while enforcing a global 429 cooldown', async () => {
    ...
    const fetch = vi.spyOn(globalThis, 'fetch').mockImplementationOnce(async () => new Response(JSON.stringify({ ok: false, error_code: 429, parameters: { retry_after: 2 } }), { status: 429 })).mockImplementation(async () => accepted());
    ...
    expect((await hub.getNotification(notification.notification.id))?.notification).toMatchObject({ status: 'completed', sent: 2, pending: 0 });
  });
  ...
  it('deactivates a blocked subscriber and skips their remaining queued notifications', async () => {
    ...
    const first = await hub.enqueue(input, source);
    const second = await hub.enqueue({ ...input, event: 'next.event' }, source);
    const fetch = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => new Response(JSON.stringify({ ok: false, error_code: 403, description: 'Forbidden' }), { status: 403 }));
    await resume();
    await runDurableObjectAlarm(hub);
    await tick();
    expect(fetch).toHaveBeenCalledTimes(1);
  ...
  it('sends long image notifications in paced photo and full-text stages', async () => {
    await subscribe(1);
    const result = await hub.enqueue({ ...input, image: 'https://example.com/alert.png', text: 'Full detail. '.repeat(100) }, source);
    ...
    expect((await hub.getNotification(result.notification.id))?.deliveries[0]).toMatchObject({ status: 'pending', stage: 1, partial: true });
    await tick(500);
    expect(fetch).toHaveBeenCalledTimes(1);
    await tick(501);
    expect(fetch).toHaveBeenCalledTimes(2);
    ...
    expect((await hub.getNotification(result.notification.id))?.deliveries[0]).toMatchObject({ status: 'sent', attempts: 2, partial: false });
  });

  it('preserves photo-partial truth when text delivery becomes uncertain', async () => {
    await subscribe(1);
    const result = await hub.enqueue({ ...input, image: 'existing_telegram_file_id', text: 'Detail '.repeat(200) }, source);
    ...
    expect((await hub.getNotification(result.notification.id))?.deliveries[0]).toMatchObject({ status: 'unknown', stage: 1, partial: true });
  });
  ...
  it('bounds retries to five attempts per stage when Telegram repeatedly rejects with 429', async () => {
    await subscribe(1);
    const result = await hub.enqueue(input, source);
    const fetch = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => new Response(JSON.stringify({ ok: false, error_code: 429, parameters: { retry_after: 1 } }), { status: 429 }));
    await resume();
    for (let count = 0; count < 7; count++) await tick();
    expect(fetch).toHaveBeenCalledTimes(5);
    expect((await hub.getNotification(result.notification.id))?.deliveries[0]).toMatchObject({ status: 'failed', attempts: 5 });
  });
...
async function tenant(limits: Partial<TenantLimits> = {}) {
  ...
  const mock = vi.spyOn(globalThis, 'fetch').mockImplementation(async () => new Response(JSON.stringify({ok: true, result: {id: botId, is_bot: true, first_name: 'Test', username: 'tenant_test_bot'}})));
...
describe('tenant isolation and durable limits', () => {
  ...
  it('counts accepted notifications durably without counting duplicates or forgetting purged records', async () => {
    ...
    await expect((async () => await hub.enqueue({...input, event: 'second'}, source))()).rejects.toThrow('DAILY_LIMIT:');
    await runInDurableObject(hub, (_, state) => state.storage.sql.exec('UPDATE notifications SET created_at = 1'));
    now += 60_001;
    await expect((async () => await hub.enqueue({...input, event: 'after-retention'}, source))()).rejects.toThrow('DAILY_LIMIT:');
  ...
  it('enforces concurrent daily admissions atomically', async () => {
    const account = await tenant({notificationsPerDay: 1});
    hub = account.hub;
    const results = await Promise.allSettled([1, 2, 3].map(async (index) => await hub.enqueue({...input, event: `concurrent.${index}`}, source)));
  ...
  it('uses tenant bot credentials and rechecks enabled state between sends', async () => {
    ...
    expect(fetch).toHaveBeenCalledTimes(1);
    expect(String(fetch.mock.calls[0][0])).toContain(`bot${account.botId}:tenant-token-${account.id}/sendMessage`);
    expect((await hub.getNotification(notification.notification.id))?.deliveries[0].error).toBeNull();
    expect((await hub.getNotification(notification.notification.id))?.notification).toMatchObject({sent: 1, pending: 1});
  });
});
...
describe('per-instance read cache', () => {
  it('caches rate-limit reads without losing durable counts or changed limits after eviction', async () => {
    ...
    await runInDurableObject(hub, (instance) => {
      ...
      expect((instance as unknown as {rateCounters: Map<string, unknown>}).rateCounters.size).toBeLessThanOrEqual(256);
    });
  });
  ...
  it('retains a short retry alarm when authoritative tenant runtime is temporarily unavailable', async () => {
    await hub.enqueue(input, source);
    await runInDurableObject(hub, async (instance, state) => {
      const runtime = vi.spyOn(instance as unknown as {runtime(): Promise<TenantRuntime>}, 'runtime').mockRejectedValueOnce(new Error('Registry temporarily unavailable'));
  ...
  it('serves repeated settings, overview, list, detail, subscribers and usage without SQL', async () => {
    ...
    const notification = await hub.enqueue(input, source);
    await runInDurableObject(hub, (instance, state) => {
      instance.getSettings(); instance.getOverview(); instance.listNotifications(1);
      instance.getNotification(notification.notification.id); instance.listSubscribers(1); instance.getUsage();
      const sql = vi.spyOn(state.storage.sql, 'exec');
...
describe('subscriber bans and application filtering', () => {
  it('bans a bounded batch, skips pending deliveries, and requires a fresh start after unban', async () => {
    ...
    await subscribe(1);
    expect((await hub.listSubscribers(1)).items.find((row) => row.chatId === '1')).toMatchObject({banned: true, banReason: 'Abuse', active: false});
    expect((await hub.enqueue(input, source)).notification.total).toBe(0);
    await hub.setSubscriberBan(['1'], false);
    expect((await hub.listSubscribers(1)).items.find((row) => row.chatId === '1')).toMatchObject({banned: false, banReason: null, active: false});
    await subscribe(1);
...
async function chooseApplications(chatId: number, data: string, from = chatId) {
  const id = ++updateId;
  await hub.handleUpdate({update_id: id, callback_query: {
...
describe('subscriber application preferences', () => {
  it('defaults to all, supports selected apps, and restores all when the final choice is removed', async () => {
    ...
    expect((await hub.listSubscribers(1)).items.find((row) => row.chatId === '1')).toMatchObject({applicationMode: 'selected', applicationIds: ['payments']});
    ...
    await chooseApplications(1, 'apps:toggle:deployments');
    expect((await hub.listSubscribers(1)).items.find((row) => row.chatId === '1')).toMatchObject({applicationMode: 'all', applicationIds: []});
  ...
  it('suppresses already queued excluded notifications and durably queues the Telegram menu', async () => {
    ...
    const menu = await runInDurableObject(hub, (_, state) => state.storage.sql.exec('SELECT rendered, system_payload FROM deliveries WHERE system_payload IS NOT NULL LIMIT 1').one());
  ...
  it('rejects forged, foreign, disabled-app and banned-user callbacks without changing preferences', async () => {
    ...
    await chooseApplications(1, 'apps:toggle:deployments');
    expect(fetch).not.toHaveBeenCalled();
    expect((await hub.listSubscribers(1)).items[0]).toMatchObject({applicationMode: 'all', applicationIds: [], banned: true});
  ...
  it('rechecks preferences after claiming a queued send and reloads selections after eviction', async () => {
    ...
    await runInDurableObject(hub, async (instance, state) => {
      const actualSync = state.storage.sync.bind(state.storage);
      const sync = vi.spyOn(state.storage, 'sync').mockImplementationOnce(async () => {
        (instance as unknown as {changeApplications(chatId: string, id: string | null): void}).changeApplications('1', 'payments');
        await actualSync();
      });
      await instance.alarm();
      sync.mockRestore();
    ...
    expect((await hub.getNotification(notification.notification.id))?.notification.skipped).toBe(1);
    await evictDurableObject(hub);
    expect((await hub.listSubscribers(1)).items[0]).toMatchObject({applicationMode: 'selected', applicationIds: ['payments']});
  });
  ...
  it('delivers /apps with an inline keyboard through the paced queue', async () => {
    ...
    expect(payload.reply_markup.inline_keyboard.flat().map((button: {callback_data: string}) => button.callback_data).sort()).toEqual(['apps:all', 'apps:toggle:deployments', 'apps:toggle:payments']);
