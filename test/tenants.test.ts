...
const response = (result: unknown) => new Response(JSON.stringify({ ok: true, result }), { headers: { 'Content-Type': 'application/json' } });
...
describe('tenant registry persistence, isolation, and cache', () => {
  ...
  it('updates authoritative runtime immediately and enforces optimistic versions', async () => {
    ...
    await expect(reject(() => registry.updateTenant('versioned', { name: 'Stale name', expectedVersion: first.tenant.version }))).rejects.toThrow('STALE_TENANT:');
...
describe('tenant bot ownership and status', () => {
  ...
  it('caches status for 60 seconds, coalesces refreshes, and invalidates after mutation', async () => {
    ...
    const fetcher = vi.spyOn(globalThis, 'fetch').mockImplementation(async request => String(request).endsWith('/getMe') ? response(bot(910005)) : response({ url: 'https://relay.test/telegram/status-cache/webhook', pending_update_count: 0 }));
  ...
  it('registers only enabled tenant webhook paths and redacts network errors', async () => {
    ...
    expect(await registry.registerWebhook('webhook', 'https://relay.test/telegram/webhook/webhook')).toEqual({ ok: true, url: 'https://relay.test/telegram/webhook/webhook' });
