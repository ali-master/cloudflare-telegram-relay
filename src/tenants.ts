...
import type { Application, ApplicationCreate, ApplicationUpdate, BotStatus, Env, LoginAuditInput, LoginAuditPage, LoginAuditQuery, Tenant, TenantCreate, TenantLimits, TenantRuntime, TenantUpdate } from './types';
...
interface StoredTenant {
  id: string; name: string; enabled: boolean; createdAt: string; updatedAt: string; version: number;
  limits: TenantLimits;
  botToken: string | null; webhookSecret: string | null;
  botId: string | null; botUsername: string | null;
}
interface TenantRow extends Record<string, SqlStorageValue> { id: string; body: string }
interface StatusEntry { signature: string; expires: number; status: BotStatus }
...
export class TenantRegistry extends DurableObject<Env> {
  ...
  listLoginAudit(query: LoginAuditQuery = {}): LoginAuditPage { return this.loginAudit.list(query); }
  ...
  async getRuntime(id: string): Promise<TenantRuntime | null> {
    const tenant = this.tenants.get(tenantId(id));
    return tenant ? { ...this.publicTenant(tenant), botToken: this.token(tenant), webhookSecret: this.secret(tenant) } : null;
  }
  ...
  async createTenant(input: TenantCreate): Promise<{ tenant: Tenant }> {
    ...
    const now = new Date().toISOString();
    const next: StoredTenant = { id, name, limits, enabled: true, createdAt: now, updatedAt: now, version: 1, botToken: null, webhookSecret: null, botId: null, botUsername: null };
    this.commit(next);
  ...
  async createApplication(tenantId: string, input: ApplicationCreate): Promise<{ application: Application; apiKey: string }> {
    this.stored(tenantId);
  ...
  async rotateApplicationKey(tenantId: string, id: string, version?: number): Promise<{ application: Application; apiKey: string }> {
    this.stored(tenantId);
  ...
  async getBotStatus(id: string, force = false): Promise<BotStatus> {
    const current = this.stored(id);
    const runtime: TenantRuntime = { ...this.publicTenant(current), botToken: this.token(current), webhookSecret: this.secret(current) };
  ...
  private async loadBotStatus(runtime: TenantRuntime): Promise<BotStatus> {
    const status: BotStatus = {
      configured: { bot: !!runtime.botToken, webhookSecret: !!runtime.webhookSecret, apiKey: this.applications.list(runtime.id).some(app => app.enabled && app.keyConfigured) },
  ...
  async registerWebhook(id: string, value: string): Promise<{ ok: true; url: string }> {
    ...
    let url: URL;
    try { url = new URL(value); } catch { fail('INVALID_TENANT', 'A public HTTPS webhook URL is required.'); }
    ...
    await this.ctx.blockConcurrencyWhile(async () => {
      try {
        ...
        const result = await telegramCall<boolean>(token, 'setWebhook', {
          url: url!.toString(), secret_token: secret, allowed_updates: ['message', 'my_chat_member', 'callback_query'], drop_pending_updates: false,
        });
